...
console.log((0, _index.default)(2, 3)); //crear funciones con promesa para validar 
//del diagrama options,boleano--usar promesa

const path = require('path');

const fs = require('fs'); // const myMarked = require('marked'); 
...
console.log(isValidPath(ruta1)); // DETERMINAR SI LA RUTA ES ABSOLUTA

const pathAbsolute = route => path.isAbsolute(route);

console.log('is absolute: ', pathAbsolute(ruta2)); //transformar ruta relativa a abasoluta

const transAbsolute = route => path.resolve(route);

console.log('transAbsolute: ', transAbsolute(ruta1)); //Es directorio
...
console.log(pathIsDirectory(ruta3)); // VERIFICAR SI ES UN ARCHIVO
...
console.log('is file: ', isFile(ruta1)); // const isFile = (route) => {
...
const readingFiles = route => fs.readFileSync(route, 'utf-8'); // función sincróna para leer los files
...
console.log(readingDirectories('C:/Users/HP/MD-links/LIM013-fe-md-links')); //devuelve array de elementos