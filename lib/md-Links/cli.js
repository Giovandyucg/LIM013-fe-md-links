...
exports.statsValidate = exports.optionStats = void 0;

var _mdLinks = require("./mdLinks");

const chalk = require('chalk'); // import {optionValidate} from './utils1'
  ...
  if (route) {
    if ((stats >= 0 || optStats >= 0) && (validate >= 0 || optValidate >= 0)) {
      return (0, _mdLinks.mdLinks)(route, {
        validate: true
      }).then(links => console.log(chalk.green(statsValidate(links)))).catch(error => console.log(chalk.bgRedBright.black(error)));
    }
    ...
    if (validate >= 0 || optValidate >= 0) {
      ...
      .then(links => links.forEach(link => console.log(chalk.green(link.file, link.href, link.statusText, link.status, link.text)))).catch(error => console.log(warning));
    }

    if (stats >= 0 || optStats >= 0) {
      return (0, _mdLinks.mdLinks)(route, {
        validate: false
      }).then(links => console.log(chalk.blueBright(optionStats(links)))).catch(error => console.error(error));
    } else {
      return (0, _mdLinks.mdLinks)(route, {
        validate: false
      }) // .then((links) => console.log(links))
      .then(links => links.forEach(link => console.log(chalk.green(link.href, link.file, link.text)))).catch(error => console.error(chalk.bgRedBright.black('Error')));
    }
  }
};
